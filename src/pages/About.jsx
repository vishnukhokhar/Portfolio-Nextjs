      ...
      <div className="fixed top-1/2 left-0 transform -translate-y-1/2 z-50">
        <ul className="p-0">
          {icons.map((icon, index) => (
            <li
              key={index}
              className="h-10 w-10 sm:h-8 sm:w-8 md:h-10 md:w-10 list-none pl-0.5 pt-0.75 mt-7 mr-4 bg-whitesmoke rounded-[30%] hover:bg-[#eaed94]"
            >
              <a href={icon.href} target="_blank" rel="noopener noreferrer">
                <img
                  src={icon.src}
                  alt={icon.alt}
                  className="h-9 w-9 sm:h-7 sm:w-7 md:h-9 md:w-9 rounded-lg"
                />
              </a>
            </li>
          ))}
